...
import io.yaochi.recommendation.model.dcn.DCN
import io.yaochi.recommendation.optim.AsyncAdam
import org.apache.spark.{SparkConf, SparkContext}

object DCNLocalExample {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val crossDepth = params.getOrElse("crossDepth", "-1").toInt
    ...
    val model = new ParRecModel(optim, new DCN(inputDim, nFields, embeddingDim, crossDepth, fcDims))
    model.init()
